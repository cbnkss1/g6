...
/** 수령인이 받은 롤링P 합(차액+본인+루징+추천). 구 API는 네 열을 더함. */
export function rollingRecvTotalString(row: RollingRecvRow): string {
  const t = String(row.rolling_recv_total ?? "").trim();
  if (t !== "" && t !== "—") return t;
  const sum =
    parse(row.rolling_paid_sum) +
    parse(row.rolling_self_sum) +
    parse(row.rolling_diff_losing_sum) +
    parse(row.rolling_referral_sum);
  return String(sum);
}
